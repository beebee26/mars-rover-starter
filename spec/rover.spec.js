...
describe("Rover class", function() {
  ...
  it("constructor sets position and default values for mode and generatorWatts", function() {
      let output = new Rover('12345', 'MOVE', 110);
...
describe("Rover class", function() {
  ...
  it("responds correctly to mode change command", function() {
     let com = [new Command('MODE_CHANGE', 'LOW_POWER')];
      ...
      let output = rov.receiveMessage(msg);
      let checkMode = rov.receiveMessage(msg)
      let checkObject = {
      'completed':true, 
      };
      assert.strictEqual(rov.mode, 'LOW_POWER')
      assert.equal(output.toString(), checkObject.toString());
   });
});
...
describe("Rover class", function() {
  ...
  it("responds with false completed value when attempting to move in LOW_POWER mode", function() {
    let rov = new Rover(98382); 
    rov.mode = 'LOW_POWER';
    let com = [new Command('MOVE', '98765')];
    let msg = new Message('Test competed = false with move change on LOW', com);
    let output = rov.receiveMessage(msg);
    str = output.results[0].toString();
    let checkObject = {
    'completed':false, 
    };
    assert.equal(output.results[0].completed, checkObject.completed)
  });
})
